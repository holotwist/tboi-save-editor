...
// CRC Table
const uint32_t SaveFile::CrcTable[256] = {
...
void SaveFile::parseSectionOffsets() {
    size_t offset = 0x14;
    std::vector<size_t> entryLens = {1, 4, 4, 1, 1, 1, 1, 4, 4, 1, 546};
    sectionOffsets.assign(entryLens.size(), 0);

    for (size_t i = 0; i < entryLens.size(); i++) {
        size_t sectCount = getInt(offset + 8, 4);
        offset += 12; // 3 * 4 bytes for the section header
        
        if (sectionOffsets[i] == 0) sectionOffsets[i] = offset;
        offset += sectCount * entryLens[i];
    }
}

int32_t SaveFile::getInt(size_t offset, size_t num_bytes) const {
    if (offset + num_bytes > data.size()) return 0;
    int32_t val = 0;
    for (size_t i = 0; i < num_bytes; ++i) val |= (data[offset + i] << (i * 8));
    return val;
}

void SaveFile::setInt(size_t offset, int32_t val, size_t num_bytes) {
    if (offset + num_bytes > data.size()) return;
    for (size_t i = 0; i < num_bytes; ++i) data[offset + i] = (val >> (i * 8)) & 0xFF;
}

// Section Indexes: 0: Achievements, 1: Stats, 3: Items, 6: Challenges
bool SaveFile::getItem(int id) const { return data[sectionOffsets[3] + id] == 1; }
void SaveFile::setItem(int id, bool unlocked) { data[sectionOffsets[3] + id] = unlocked ? 1 : 0; }

bool SaveFile::getAchievement(int id) const { return data[sectionOffsets[0] + id] == 1; }
void SaveFile::setAchievement(int id, bool unlocked) { data[sectionOffsets[0] + id] = unlocked ? 1 : 0; }

bool SaveFile::getChallenge(int id) const { return data[sectionOffsets[6] + id] == 1; }
void SaveFile::setChallenge(int id, bool unlocked) { data[sectionOffsets[6] + id] = unlocked ? 1 : 0; }

int32_t SaveFile::getStat(size_t internalOffset) const {
    return getInt(sectionOffsets[1] + 0x4 + internalOffset, 4);
}

void SaveFile::setStat(size_t internalOffset, int32_t amount) {
    setInt(sectionOffsets[1] + 0x4 + internalOffset, amount, 4);
}

uint32_t SaveFile::calcChecksum(size_t ofs, size_t length) const {
    uint32_t checksum = ~0xFEDCBA76;
    for (size_t i = ofs; i < ofs + length; ++i)
        checksum = CrcTable[(checksum ^ data[i]) & 0xFF] ^ (checksum >> 8);
    return ~checksum;
}

void SaveFile::updateChecksum() {
    size_t offset = 0x10;
    size_t length = data.size() - offset - 4;
    uint32_t checksum = calcChecksum(offset, length);
    setInt(offset + length, checksum, 4);
}